 ...
 */
export const vk = async (signing_key) => {
 ...
 */
export const sign = async (message, signing_key) => {
 ...
 */
export const verify = async (signature, message, verifying_key) => {
    const bytes = new TextEncoder().encode(message)
    ...
    (async () => {
        ...
        switch(process.argv[2]) {
            case 'sk':     
                res = sk(); break;
            case 'vk':     
                res = await vk(process.argv[3]);  break;
            case 'sign':   
                res = await sign(process.argv[3], process.argv[4]); break;
            case 'verify': 
                res = await verify(process.argv[3], process.argv[4], process.argv[5]); break;
            default:     
                res = '';
        }
